...
4. Use a tag with the `v` prefix, for example `v1.0.0-B10`.
...
The script writes generated output under `artifacts\` and normal `bin\`/`obj\` build folders; all are ignored by Git:
...
git tag -a v1.0.0-B10 -m "Release Android TV Manager 1.0.0-B10"
git push origin v1.0.0-B10
```
...
The same workflow can be rerun from GitHub Actions with `workflow_dispatch` by supplying an existing version tag. Existing release assets are replaced with `--clobber` and the release notes are refreshed; a second release is not created.
